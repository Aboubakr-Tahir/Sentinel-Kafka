error id: file://<WORKSPACE>/src/main/scala/AlertSystem.scala:
file://<WORKSPACE>/src/main/scala/AlertSystem.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -org/apache/kafka/streams/scala/kstream/metric/component_id.
	 -org/apache/kafka/streams/scala/kstream/metric/component_id#
	 -org/apache/kafka/streams/scala/kstream/metric/component_id().
	 -io/circe/metric/component_id.
	 -io/circe/metric/component_id#
	 -io/circe/metric/component_id().
	 -io/circe/generic/auto/metric/component_id.
	 -io/circe/generic/auto/metric/component_id#
	 -io/circe/generic/auto/metric/component_id().
	 -io/circe/parser/metric/component_id.
	 -io/circe/parser/metric/component_id#
	 -io/circe/parser/metric/component_id().
	 -io/circe/syntax/metric/component_id.
	 -io/circe/syntax/metric/component_id#
	 -io/circe/syntax/metric/component_id().
	 -org/apache/kafka/streams/scala/ImplicitConversions.metric.component_id.
	 -org/apache/kafka/streams/scala/ImplicitConversions.metric.component_id#
	 -org/apache/kafka/streams/scala/ImplicitConversions.metric.component_id().
	 -org/apache/kafka/streams/scala/serialization/Serdes.metric.component_id.
	 -org/apache/kafka/streams/scala/serialization/Serdes.metric.component_id#
	 -org/apache/kafka/streams/scala/serialization/Serdes.metric.component_id().
	 -metric/component_id.
	 -metric/component_id#
	 -metric/component_id().
	 -scala/Predef.metric.component_id.
	 -scala/Predef.metric.component_id#
	 -scala/Predef.metric.component_id().
offset: 3587
uri: file://<WORKSPACE>/src/main/scala/AlertSystem.scala
text:
```scala
package sentinel
...
import org.apache.kafka.streams.kstream.{Windowed, TimeWindows, Suppressed}
...
object AlertSystem extends App {
  ...
  parsedStream.filter { (key, metric) =>
    ...
    (metric.co@@mponent_id == "RAM" && metric.value > 50.0)
  }.foreach { (key, metric) =>
    logger.error(s"ALERTE IMMEDIATE [SANTE SERVEUR]: ${metric.component_id} critique à ${metric.value}% !")
  }
  ...
  val windowedStats = groupedStream
    .windowedBy(TimeWindows.ofSizeAndGrace(Duration.ofSeconds(40), Duration.ofSeconds(1)))
  ...
  windowedStats
    ...
    .foreach { (windowedKey, agg) =>
      ...
      if (key == "CPU_TOTAL" && agg.average > 85.0) {
        logger.error(s"ALERTE CONFIRMEE [CHARGE GLOBALE]: Le serveur sature (Moyenne CPU_TOTAL: ${agg.average}% sur 5 min).")
      }
      ...
      if (key.startsWith("CPU_") && key != "CPU_TOTAL") {
        ...
        if (agg.average > 90.0) {
          logger.warn(s"DIAGNOSTIC [BUG APPLICATIF]: Le coeur $key est saturé (${agg.average}%). Vérifier les threads.")
        }
        // Instabilité (Flapping) sur un cœur spécifique
        if (agg.max - agg.min > 80.0) {
           logger.warn(s"DIAGNOSTIC [INSTABILITE]: Le coeur $key oscille violemment (${agg.min}% - ${agg.max}%).")
        }
      }
    }
  ...
  groupedStream
    ...
    .foreach { (windowedKey, agg) =>
         logger.warn(s"SUSPICION MEMORY LEAK: Usage RAM moyen élevé (${agg.average}%) sur la dernière heure.")
    }
